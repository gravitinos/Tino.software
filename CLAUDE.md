...
- In Claude Code on the web, set `AGENT_BROWSER_EXECUTABLE_PATH=/opt/pw-browsers/chromium` (Chrome for Testing can't be downloaded there). `record` needs `ffmpeg` (`apt-get install -y ffmpeg`).
